...
5. Run the project:
    ...
    It will be run on a machine with GPU (`--gpu`), using a Tenserflow environment (`--env tensorflow`), as a Jupyter notebook (`--mode jupyter`), input data will be taken from `R5KrjnANiKVhLWAkpXhNBe` which is already hosted at `floydhub`.
